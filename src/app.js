...
const createServer = async (serverData) => {
  try {
    console.log(
      "Attempting to create server with data: ",
    ...
    if (!response.ok) {
      ...
      if (data.errors) {
        console.error(
          "Validation errors: ",
          data.errors.map((error) => JSON.stringify(error, null, 2)).join("\n")
        );
      }
      throw new Error(
...
const getNestAndEggIds = async () => {
  try {
    ...
    if (!nestsResponse.ok)
      throw new Error(JSON.stringify(nestsData) || nestsResponse.statusText);
    ...
    if (!eggsResponse.ok)
      throw new Error(JSON.stringify(eggsData) || eggsResponse.statusText);
    ...
    console.log(
      "Nest and egg IDs fetched successfully. Nest ID: ",
      nestId,
      ", Egg ID: ",
      eggId
    );
    ...
    console.error(
      "Error fetching nest and egg IDs: ",
      typeof error === "object" ? JSON.stringify(error, null, 2) : error
    );
    throw error;
  }
...
const getAvailableAllocationId = async () => {
  try {
    ...
    if (!allocationsResponse.ok)
      throw new Error(
        JSON.stringify(allocationsData) || allocationsResponse.statusText
      );
    ...
    console.log(
      "Available allocation ID fetched successfully: ",
      availableAllocation.attributes.id
    );
    return availableAllocation.attributes.id;
  } catch (error) {
    console.error(
      "Error fetching available allocation ID: ",
      typeof error === "object" ? JSON.stringify(error, null, 2) : error
    );
    throw error;
  }
...
app.post("/create-server", async (req, res) => {
  try {
    ...
    console.error(
      "Error in /create-server endpoint: ",
      typeof error === "object" ? JSON.stringify(error, null, 2) : error
    );
    res.status(500).json({ error });
  }
